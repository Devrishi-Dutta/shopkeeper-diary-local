...
const Sellers = () => {
    ...
    const getSellers = async () => {
        ...
        let result = await fetch(`${process.env.REACT_APP_API}/sellers/${userId}`,{
            headers:{
    ...
    const deleteSeller = async (id) => {
        let result = await fetch(`${process.env.REACT_APP_API}/seller/${id}`, {
            method: "Delete",
            headers:{
                authorization:`bearer ${JSON.parse(localStorage.getItem('token'))}`
            }
    ...
    const searchHandle= async (e)=>{
        ...
        if(key){
            let result = await fetch(`${process.env.REACT_APP_API}/search2/${key}`,{
                headers:{
                    authorization:`bearer ${JSON.parse(localStorage.getItem('token'))}`
                }
            });
